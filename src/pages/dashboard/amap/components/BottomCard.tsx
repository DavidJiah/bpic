...
const BottomCard: React.FC<any> = (props) => {
  ...
  let ecolumns = [
    { title: '信号灯型号', key: 'text', dataIndex: 'b1' },
    { title: '信控机机型', key: 'text', dataIndex: 'b2' },
  ];
  equipment.forEach((item: any, index: number) => {
    ecolumns.push({ title: item.equipmentType, key: 'text', dataIndex: `a${index}`, span: '24' });
    detail[
      `a${index}`
    ] = `北（${item.northNumber}）南（${item.southNumber}）东（${item.eastNumber}）西（${item.westNumber}）`;
  });
  ...
  return (
    <Row gutter={16} className={styles.RowBox}>
      <Col span={8} style={{ paddingRight: '0px!important' }}>
        <Card className={styles.Card} onClick={() => redirect('/eam/intersection')}>
          <ProDescriptions
            ...
            columns={[
              ...
              },
              {
                ...
                render: () => {
                  return intImportanceLevelEnum[intersection.intImportanceLevel];
                },
              },
              {
                title: '路口形状',
                ...
                render: () => {
                  return intIntersectionLatitudeEnum[intersection.intIntersectionShape];
                },
              },
              {
                title: '交通状态',
      ...
      <Col span={8} style={{ paddingRight: '0px!important' }}>
        <Card className={styles.Card} onClick={() => redirect('/eam/facilities')}>
          <ProDescriptions title="设施设备管理" dataSource={detail} column={2} columns={ecolumns} />
        </Card>
      </Col>
      <Col span={8}>
        <Card className={styles.Card} onClick={() => redirect('/eam/fileAndImg')}>
          <p>路口图片集</p>
          {imageList &&
