...
class Amap extends Component<AmapProps> {
  ...
  get IntersectionModalProps() {
    const { dispatch,amap } = this.props
    const {currentItem, modalVisible, modalType } = amap
  ...
  get MarkerProps(){
    ...
    return {
      list: intersections.map((e: any)=>{return { controlPlan:e.controlPlan,id: e.intIntersection.id, title: e.intIntersection.intIntersectionName, lnglat: [e.intIntersection.intIntersectionLatitude, e.intIntersection.intIntersectionLongitude] }}),
      onDetail(row: any) {
        dispatch({
  ...
  render() {
    ...
    const {HeaderProps,MarkerProps,IntersectionModalProps,RoadMarkerProps,BottomCardProps } = this
