...
import {
  queryControlStrategyList,
  saveControlStrategy,
  delControlStrategy,
  updataControlStrategy,
  queryControlPlanList,
} from '../service';
  ...
  const columns: ProColumns<any>[] = [
    {
      ...
      render: (_, row) => {
        return `${row?.comBeginTime ? toHHmmss(row.comBeginTime) : ''}-${
          row?.comEndTime ? toHHmmss(row.comEndTime) : ''
        }`;
      },
    },
    {
        ...
        editable={{
          ...
          onSave: async (key, row) => {
            ...
            if (!model.id) {
              ...
              if (msg.code === 0) message.success('更新成功');
              else message.error('更新失败，请联系管理员');
            }
