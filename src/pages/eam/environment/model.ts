...
import { queryList,queryEnvFactor,deleteEnvFactor,queryFilePage } from './service';
  ...
  effects: {
    ...
    *showCard({ payload }, { call, put }) {
      ...
      if(list.length>=1){
        ...
        const resFile = yield  call(queryFilePage, { conditions:[{column:'businessId',option:'in',valueList}],size:-1,template: {groupCode: 'Default',businessType:'envfactor',moduleCode:'uscp'}});
        list = list.map((item1: any)=>{
          item1.fileList=resFile.data.records.filter((item: any)=>item.businessId===`${item1.intIntersectionId}-${item1.id}`).map((fitem: any)=>{ return {url: `http://116.63.45.33:8888/file/download/${fitem.id}`}})
          return item1
        })
      }
      yield put({
    ...
    *delete({ payload }, { call, put }) {
      ...
      yield put({
        type: 'queryCardData',
        payload:  {list:response.data ? response.data:[],intersectionId:payload.intersectionId}
      });
    }
  },
  reducers: {
