 ...
 * @LastEditors: Dad
 * @LastEditTime: 2021-03-23 14:20:41
 */
...
const Comp: React.FC<FileAndImgProps> = (props) => {
        ...
        <ProCard split="vertical" className={styles.bg}>
          <ProCard title="益阳市" colSpan="260px" className={styles.bg}>
            <RoadTree
              onSelect={(_: any, e: any) =>
                e?.node?.item1?.intersectionCode && setBusinessId(e?.node?.item1?.intersectionCode)
              }
            />
          </ProCard>
          <ProCard className={styles.bg}>
            ...
            <div style={{ margin: '20px 0' }}>
              {manage == 'image' ? (
                <Image businessId={businessId} {...props} />
              ) : (
                <File businessId={businessId} {...props} />
              )}
            </div>
          </ProCard>
        </ProCard>
