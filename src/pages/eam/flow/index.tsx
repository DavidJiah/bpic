...
// 时间戳转时分
const timestamp_to_time = (date: any) => {
...
const TableList: React.FC<{}> = () => {
  ...
  const handleRemove = async (selectedRows: any) => {
    const msg = await BaseApi(`/api/biz/intersection/${selectedRows[0].intIntersectionId}/traffic-volume/`).removeById(selectedRows[0].id)
    if (msg.code === 0) {
  ...
  const AmplificationModalVisible = (prop: any) => {
    ...
    return (
      ...
      >
        <div ref={amplification} style={{ width: '500px', height: '500px', background: '#141414', padding: '20px', boxSizing: 'content-box' }} />
      </Modal>
    )
  }
  ...
  const expandedRowRender = (prop: any) => { // 展开行
    return (
      <>
        <Row gutter={[30, 0]}>
          <Col span={7}>
            <div
              ...
              onClick={() => { handleAmplificationModalVisible(true), setAmplificationData(prop.trafficVolumeDetailList) }}
            />
          </Col>
          <Col span={17}>
            <ProTable
  ...
  const onChange = (info: any) => { // 导入流量数据反馈
          ...
          <ProCard title={intersectionTitle} >
            <ProTable<any>
              ...
              toolBarRender={() => [
                ...
                <Upload name='trafficVolumeFile' onChange={onChange} showUploadList={false} action='/api/office/traffic-volume/excel-import'>
                  <Button type="dashed" onClick={() => { }}>导入</Button>
                </Upload>
              ]}
              request={async (params) => {
                const newParams = { conditions: [] as any[], size: params?.pageSize, current: (params?.current && params?.current - 1) }
                for (const key in params) {
                  if (key !== 'current' && key !== 'pageSize') {
                    if (Array.isArray(params[key]) && params[key].length === 2) {
                      if (key === 'timeRange') {
                        newParams.conditions.push(
                          { column: 'comBeginTime', option: 'between', value: params?.timeRange[0], value2: params?.timeRange[1] },
                          { column: 'comEndTime', option: 'between', value: params?.timeRange[0], value2: params?.timeRange[1] }
                        )
                      } else {
                        newParams.conditions.push({ column: key, option: 'between', value: params[key][0], value2: params[key][1] })
                      }
                    } else {
                ...
                const msg = await BaseApi(`/api/biz/intersection/${intersectionId}/traffic-volume/`).page(newParams)
                const newmsg = JSON.parse(JSON.stringify(msg.data.records))
                newmsg.map((item: any) => {
                  item.timeRange = `${timestamp_to_time(item?.comBeginTime)}~${timestamp_to_time(item?.comEndTime)}`
                })
                return {
                  data: newmsg,
                  success: true,
