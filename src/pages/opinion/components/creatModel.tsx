...
const FormModal = (props: any) => {
  ...
  const creatdata = async (formData: any) => {
    ...
    if (code === 0)
      message.success('保存成功'), await saveForm(Formdata, data.id), onCancel(), initList();
    else message.error(msg);
  };

  const editdata = async (formData: any) => {
    formData.takingTime = new Date(formData.takingTime).getTime();
    const { code, msg } = await uplaod(formData, editData?.id);
    ...
    <>
      ...
      >
        ...
        >
          <ProFormText
            width="md"
            name="publicSentimentCode"
            label="舆情编号"
            placeholder="请输入舆情编号"
            rules={[{ required: true, message: '请输入舆情编号' }]}
          />
          <ProForm.Group>
            <ProFormSelect
              width="md"
              ...
              label="舆情来源"
              initialValue="市长热线"
              request={async () => [{ label: '市长热线', value: '市长热线' }]}
          ...
          <ProForm.Group>
            <ProFormSelect
              ...
              request={async () =>
                _.map(OPINION_TYPE, (e: any) => ({ label: e.value, value: e.value }))
              }
            />
            <ProFormSelect
              width="md"
              ...
              request={async () => {
                ...
                return _.map(data, (e: any) => ({
                  label: e.intersectionName,
                  value: e.intersectionName,
                }));
              }}
            />
          </ProForm.Group>
          <Form.Item name="publicSentimentFile" label="舆情单上传">
            <GlobalUpload
              imgUrl={''}
