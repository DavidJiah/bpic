...
import { Modal, Form, message, Checkbox, Divider } from 'antd';
import ProForm, { ProFormDateRangePicker, ProFormCheckbox } from '@ant-design/pro-form';
...
const ModalForm = (props: any) => {
  ...
  const getIntersectionInfo = async() => {
    ...
    if(code === 0) setcheckboxGroup(_.map(data, (item: any) => ({ label: item?.intIntersection?.intIntersectionName, value: item?.intIntersection?.id })))
  ...
  const creatdata = async (formData: any) => {
    const { code, msg } = await creatData(formData)
