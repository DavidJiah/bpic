...
const ModalForm = (props: any) => {
  ...
  useEffect(() => {
    getIntersectionInfo();
    editData.dateRange = [moment(moment(editData?.inspScheduleBeginDate).format(Day), Day), moment(moment(editData?.inspScheduleEndDate).format(Day), Day)]
  ...
  const creatdata = async (formData: any) => {
    ...
    if (code === 0) message.success('新增成功'), initList()
  ...
  const editdata = async (formData: any) => {
    ...
    if (code === 0) message.success('更新成功'), initList()
    else message.error(msg);
  };
