...
import { getpages, getIntersectionInfo, Delete, submitAudit } from './service'
...
const PatrolTask: React.FC<CompProps> = ({ dispatch, loading }) => {
  ...
  const expandedRowRender = (props: any) => {
    ...
    return (
      <div>
        <Table columns={columns} dataSource={props?.dailyTaskIntersectionList} pagination={false} />
      </div>
    )
  }
  ...
  */
  ...
  const initList = async () => {
    ...
    const list = _.map(data?.records, (item: any) => ({
      ...item?.dailyTask,
      dailyTaskIntersectionList: item?.dailyTaskIntersectionList,
    }))
  ...
  const menu = (data: any) => {
    return (
      <Menu>
        ...
        {data?.inspTaskStatus == '已驳回'?<Menu.Item key="2" onClick={() => handleEidt(data)}>驳回原因</Menu.Item>:null}
        {data?.inspTaskStatus == '未提交'?
          <Menu.Item key="3">
  ...
  const submitaudit = async(record: any) => {
    const {code, msg} = await submitAudit(record?.id)
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (text: any, record: any) => (
        <Space>
          {
            record?.inspTaskStatus != '已驳回'?
            <a onClick={() => submitaudit(record)}>提交审核</a> :
            <a onClick={() => submitaudit(record)}>重新提交</a>
          }
  ...
  return (
    <div>
      <Form {...layout} name="formList" form={form} initialValues={{ remember: true }}>
        <Row>
          {/* <Col span={2}>
            <Button type="primary" shape="round" onClick={() => { history.push({pathname: '/patrol2/intersectionPatrolList'}) }}>新增巡检</Button>
          </Col> */}
          <Col span={6}>
